...
from object_tracker_3d_visualizer import ObjectTracker3DVisualizer  # Add this import
...
def main():
    ...
    if roi_selected:
        ...
        # Set the ROI size in the visualizer
        visualizer.set_roi_size(w_roi, h_roi)
    ...
    while True:
        try:
            ...
            if center_orig is not None and bbox_orig is not None:
                ...
                x_roi, y_roi, w_roi, h_roi = tracker.roi
                x_relative = center_orig[0] - x_roi
                y_relative = center_orig[1] - y_roi
                
                # Update 3D visualizer with the relative position
                visualizer.update_position(x_relative, y_relative)

                # Draw bounding box on original frame
                x_orig, y_orig, w_orig, h_orig = bbox_orig
                cv2.rectangle(frame, (x_orig, y_orig), (x_orig + w_orig, y_orig + h_orig), (0, 255, 255), 2)

                # Draw current position on original frame
                cv2.circle(frame, center_orig, 5, (0, 255, 255), -1)
            ...
            if tracker.roi:
                 ...
                 max_coords_text = f"Max Coords: ({w_roi - 1}, {h_roi - 1})"
                 text_size = cv2.getTextSize(max_coords_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)[0]
                 text_y = new_height - text_size[1] - 10
                 cv2.putText(resized_roi_frame, max_coords_text,
                             (10, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
